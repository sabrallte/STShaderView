...
vec2 mapSnail( vec3 p, out vec4 matInfo )
{
    ...
    // disp
    float dis = texture2D( iChannel1, 5.0*p.xy ).x;
...
vec3 shadeOpaque( in vec3 ro, in vec3 rd, in float t, in float m, in vec4 matInfo )
{
	...
	else if( m<2.5 ) // shell
    {
        ...
        vec3 ral = texture2D( iChannel1, vec2(2.0*matInfo.w+matInfo.z*0.5,0.5) ).xxx;
        mateD *= 0.25 + 0.75*ral;
    ...
    else if( m<3.5 ) // plant
    {
        ...
        vec3 te = texture2D( iChannel2, pos.xy*0.2 ).xyz;
        mateS *= 0.5 + 1.5*te;
    ...
    else //if( m<4.5 ) // leave
    {
        ...
        vec3 te = texture2D( iChannel2, p.xz*0.35 ).xyz;
        ...
        nor.xz += v*0.15*(-1.0+2.0*texture2D( iChannel3, 1.0*p.xz ).xy);
        nor = normalize( nor );
...
vec3 shadeTransparent( in vec3 ro, in vec3 rd, in float t, in float m, in vec4 matInfo, in vec3 col, in float depth )
{
    ...
    col += vec3(0.8,1.0,0.8)*0.5*smoothstep(0.3,0.6,texture2D( iChannel1, 0.8*nor.xy ).x)*(0.1+0.9*fre*fre);
